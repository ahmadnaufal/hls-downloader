...
class PlaylistFetcherThread(threading.Thread):
  def __init__(self, name, playlist_url, output_dir):
  ...
  def run(self):
    while True:
      ...
      for segment in playlist.segments:
        ...
        ts_props = (download_uri, filename)
        if filename in hmap:
...
class DownloaderThread(threading.Thread):
  ...
  def dequeue_download(self):
    download_uri, filename = q.get(block=True, timeout=30)

    success = False
    for i in range(self.num_retries):
      resp = requests.get(download_uri)
      logging.info("Consumer - Request to %s (Retries: %d)" % (download_uri, i))
...
def main():
  ...
  downloader = DownloaderThread("downloader", args.odir)
  fetcher = PlaylistFetcherThread("fetcher", args.playlist_url, args.odir)
